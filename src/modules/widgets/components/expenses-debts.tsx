...
export const ExpensesDebts = ({ expenses, groupUsers, loggedInUser, onSettleDebt }: ExpensesDebtsProps) => {
...
<span className="text-xs text-muted-foreground">
    → {groupUsers.find(u => u.id === creditorId)?.name ?? creditorId}: ${amount.toFixed(2)}
                            ...
                            {loggedInUser.id === debtorId && (
                                <Button type="button" variant="outline" size="sm" onClick={() => onSettleDebt(creditorId, amount)}>
                                    Pay debt
                                </Button>
                            )}
                        </div>
