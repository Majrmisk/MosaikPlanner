...
export const ExpensesChart = ({ expenses, groupUsers }: ExpensesChartProps) => {
    ...
    const PAYER_COLORS = ['#2563eb', '#16a34a', '#dc2626', '#d97706', '#7c3aed', '#0891b2'];

    const chartData = calcSpent(expenses);

    const allPayerIds = [...new Set(chartData.flatMap((m) => m.byPayer.map((p) => p.payerId)))];
...
const calcSpent = (expenses: Expense[]): MonthlySpending[] => {
    ...
    const byMonth = Object.groupBy(
        filtered,
        (e) => format(new Date(e.payedAt), 'yyyy-MM'), // has to be formated like this for sorting
    );

    return Object.entries(byMonth)
        .map(([month, exps]) => {
            const monthExpenses = exps ?? [];
            const byPayer = Object.entries(Object.groupBy(monthExpenses, (e) => e.payedBy)).map(
                ([payerId, payerExps]) => ({
                    payerId,
